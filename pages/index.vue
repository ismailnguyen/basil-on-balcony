<template>
  ...
  <div class="columns">
    ...
    <main class="column">
      ...
      <div class="columns is-multiline">
        <div class="column is-6" v-for="(chart, index) in charts" :key="index">
          <div class="panel">
            <p class="panel-heading">
              {{ chart.options.chart.title }}
            </p>
    ...
    async asyncData ({ $axios }) {
      ...
      return {
        ...
        charts: [
          {
            type: 'PieChart',
            data: [[ "Group", "Amount"]].concat(Array.from(budgets.filter(b => b.amount).map(b => ({ group: b.group, amount: b.amount})).reduce(
  (m, {group, amount}) => m.set(group, (m.get(group) || 0) + amount), new Map
), ([group, amount]) => ({group, amount})).map(b => [b.group, b.amount])),
            options: {
              chart: {
                title: 'Budget groups',
                subtitle: 'Groups repartitions'
              },
              is3D: true,
            }
          },
          {
            type: 'PieChart',
            data: [[ "Category", "Amount"]].concat(budgets.filter(b => b.amount).map(b => [b.category, b.amount])),
            options: {
              chart: {
                title: 'Budget categories',
                subtitle: 'Categories repartition'
              },
              pieHole: 0.4
            }
          }
